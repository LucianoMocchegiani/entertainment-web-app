...
export const getMovieDetailFirebase=  async (id)=>{
    ...
    try {
        let response = { success:false, message:'Reintente nuevamente en unos momentos' };
        if(typeof Number(id) !== 'number'){
            response = { success:false, message:'id no es valido' };
            return response;
        }
        ...
        if (requestSnapshotCache.exists()){
            ...
            response = { success:true, message:`Datos obtenidos desde el cache movie ${id}`, data: movieData};
        }else{
            const requestSnapshot = await getDoc(selectedDoc)
            if(requestSnapshotCache.exists()){
                const movieData = { ...requestSnapshot.data(), id: requestSnapshot.id };
                response = { success:true, message:`Datos obtenidos desde el servidor movie ${id}`, data: movieData};
            }else{
                response = { success:false, message:`No existe el documento movie ${id}`, data:null};
            }
