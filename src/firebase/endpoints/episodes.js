...
export const getEpisodeDetailFirebase=  async (id)=>{
    ...
    try {
        ...
        if (requestSnapshotCache.exists()){
            ...
            response = { success:true, message:`Datos obtenidos desde el cache episode ${id}`, data: movieData};
        }else{
            const requestSnapshot = await getDoc(selectedDoc)
            if(requestSnapshotCache.exists()){
                const movieData = { ...requestSnapshot.data(), id: requestSnapshot.id };
                response = { success:true, message:`Datos obtenidos desde el servidor episode ${id}`, data: movieData};
            }else{
                response = { success:false, message:`No existe el documento episode ${id}`, data:null};
            }
        }
        return response
    } catch (error) {
