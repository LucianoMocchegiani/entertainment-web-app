...
export const putUser=  async (data)=>{
    try {
        ...
        // let response = { success:false, message:'Reintente nuevamente en unos momentos' };
        // data = {...data , updated_date:Timestamp.now(), created_date:Timestamp.now()}
        // const selectedDoc = setDoc(db, `users/`+data.email+'/'+data.id);
        // const resolved = await setDoc(selectedDoc, data)
        // response = { success:true, message:'Usuario cargado a firebase', data: resolved};
        // console.log(response)
        // return response
...
export const getUserDetail=  async (id)=>{
    try {
        ...
        if (requestSnapshotCache.exists()){
            ...
            response = { success:true, message:`Datos obtenidos desde el cache user ${id}`, data: movieData};
        }else{
            const requestSnapshot = await getDoc(selectedDoc)
            if(requestSnapshotCache.exists()){
                const movieData = { ...requestSnapshot.data(), id: requestSnapshot.id };
                response = { success:true, message:`Datos obtenidos desde el servidor user ${id}`, data: movieData};
            }else{
                response = { success:false, message:`No existe el documento user ${id}`, data:null};
            }
        }
        return response
    } catch (error) {
